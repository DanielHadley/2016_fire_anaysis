...
#### Now start building the data frame with a series of for loops ####
...
for (i in DF$row.number){
  ...
  dest <- gsub(" ", "", dest)
  url <- paste("https://maps.googleapis.com/maps/api/distancematrix/json?origins=",
               
               # This is what you need to replace
               "651+Somerville+Ave+Somerville+MA",
               
               "&destinations=",
               dest,
               "&mode=driving&departure_time=now&traffic_model=best_guess&key=",
               your_api_key, sep="")
  
  json_output <- fromJSON(url)
  a <- json_output$rows[[c(1,1)]]
  a$row.number <- i # include in temp. df 'a' the index number of the row from DF
  
  DF$minutes[match(a$row.number, DF$row.number)] <- a$duration$value / 60 # use the index number as a matching key to input/update the value of the variable 'minutes'
  
  DF$minutes_traffic[match(a$row.number, DF$row.number)] <- a$duration_in_traffic$value / 60 # use the index number as a matching key to input/update the value of the variable 'minutes'
}
...
toCombine <- DF %>% 
  mutate(from.Lowell = minutes,
         from.Lowell_in_traffic = minutes_traffic) %>% 
  select(Full.Address, from.Lowell, from.Lowell_in_traffic)

base_DF_Final <- merge(base_DF, toCombine)

write.csv(base_DF_Final, "./data/Fire_drive_times_w_traffic.csv")
...
toCombine <- DF %>% 
  mutate(from.HQ = minutes,
         from.HQ_in_traffic = minutes_traffic) %>% 
  select(Full.Address, from.HQ, from.HQ_in_traffic)
...
for (i in DF$row.number){
  ...
  url <- paste("https://maps.googleapis.com/maps/api/distancematrix/json?origins=",
               ...
               # This is what you need to replace
               "515+Somerville+Ave+Somerville+MA",
...
toCombine <- DF %>% 
  mutate(from.FiveFifteen = minutes,
         from.FiveFifteen_in_traffic = minutes_traffic) %>% 
  select(Full.Address, from.FiveFifteen, from.FiveFifteen_in_traffic)
...
for (i in DF$row.number){
  ...
  url <- paste("https://maps.googleapis.com/maps/api/distancematrix/json?origins=",
               ...
               # This is what you need to replace
               "146+Washington+St+Somerville+MA",
...
toCombine <- DF %>% 
  mutate(from.JoyWashington = minutes,
         from.JoyWashington_in_traffic = minutes_traffic) %>% 
  select(Full.Address, from.JoyWashington, from.JoyWashington_in_traffic)
...
for (i in DF$row.number){
  ...
  url <- paste("https://maps.googleapis.com/maps/api/distancematrix/json?origins=",
               ...
               # This is what you need to replace
               "265+Highland+Ave+Somerville+MA",
...
toCombine <- DF %>% 
  mutate(from.Highland = minutes,
         from.Highland_in_traffic = minutes_traffic) %>% 
  select(Full.Address, from.Highland, from.Highland_in_traffic)
...
toCombine <- DF %>% 
  mutate(from.Teele = minutes,
         from.Teele_in_traffic = minutes_traffic) %>% 
  select(Full.Address, from.Teele, from.Teele_in_traffic)
...
for (i in DF$row.number){
  ...
  url <- paste("https://maps.googleapis.com/maps/api/distancematrix/json?origins=",
               ...
               # This is what you need to replace
               "243+Somerville+Ave+Somerville+MA",
...
toCombine <- DF %>% 
  mutate(from.Union = minutes,
         from.Union_in_traffic = minutes_traffic) %>% 
  select(Full.Address, from.Union, from.Union_in_traffic)
...
# This is old from when I did just a subet of calls in E1 and E3

# ### Geo Drive Time Analysis Data Prep ####
# # This should hopefully get us a simple model of resonse times from various locations
# 
# # We will filter it down to E1 and E3 calls, which are mostly south east
# # We just do ones where they are first responder to compare to actual times
# # And we filter the dates because E1 moved to Lowell
# # And then things got weird in 2015
# 
# # THis will be the base that we add data columns to
# set.seed(124)
...
# 
# 
# write.csv(base_DF_Final, "./data/Fire_sample_drive_times.csv")
